...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def para(self):
        # 对每个参数的值判断是否为空
        # C参数的设置
        if not self.c_edit.text().strip():
            self.C = 1.0
        else:
            self.C = float(self.c_edit.text())
        
        # kernel参数的设置
        self.kernel = str(self.ke_box.itemText(self.ke_box.currentIndex()))
        print (self.kernel)
        # degree参数的设置
        if not self.de_edit.text().strip():
            self.degree = 3
        else:
            self.degree = int(self.de_edit.text())
        
        # gamma参数的设置
        if not self.rs_edit.text().strip():
            self.gamma = 'auto'
        else:
            self.gamma = float(self.rs_edit.text())
        
        # coef0参数的设置
        if not self.co_edit.text().strip():
            self.coef0 = 0.0
        else:
            self.coef0 = float(self.co_edit.text())
        
        # probability参数的设置
        if self.pr_box.itemText(self.pr_box.currentIndex()) == 'False':
            self.probability = False
        else:
            self.probability = True
        
        # shrinking参数的设置
        if self.sh_box.itemText(self.sh_box.currentIndex()) == 'True':
        ...
        if not self.to_edit.text().strip():
            ...
            self.tol = float(self.to_edit.text())
        
        # cache_size参数的设置
        if not self.ca_edit.text().strip():
            self.cache_size = 2
        else:
            self.cache_size = float(self.ca_edit.text())
        
        # class_weight参数的设置
        if not self.cl_edit.text().strip():
            self.class_weight = None
        else:
            self.class_weight = float(self.cl_edit.text())
        
        # verbose参数的设置
        if self.ve_box.itemText(self.ve_box.currentIndex()) == 'False':
            self.verbose = False
        else:
            self.verbose = True
        ...
        if not self.cl_edit.text().strip():
            ...
            self.max_iter = int(self.cl_edit.text())
        
        # decision_function_shape参数的设置
        if self.df_box.itemText(self.df_box.currentIndex()) == 'None':
            self.decision = None
        else:
            self.decision = self.df_box.itemText(self.df_box.currentIndex())
